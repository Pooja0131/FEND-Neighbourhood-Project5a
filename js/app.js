function appViewModel() {
    ...
    function createMarker(place) {
        ...
        Also animate the selected marker for brief time, change its icon and
        open its infoWindow.*/
    ...
    function getGoogleContent(place, callback) {
        ...
        service.getDetails(request, function(results, status) {
            ...
            if (status == google.maps.places.PlacesServiceStatus.OK) {
                if (results.website) {
                    locName = "<h4><a target='_blank' href=" + results.website + ">" + place.name + "</a></h4><br>";
                } else {
        ...
        $.ajax({
            ...
            'success': function(data, textStats, XMLHttpRequest) {
                if (data.businesses[0]) {
                    locName = "<h4><u><a target='_blank' href=" + data.businesses[0].url + ">" + data.businesses[0].name + "</u></a></h4><br>";
    ...
    /*Array which contains each place that met the search
    ...
    /*Takes user's input in search bar and compares each word against the name and
     type(restaurant, bar etc) of each place in allPlaces. If its a match that
     place is pushed into filterPlace array and its marker is created.
     Then the filterd places are pushed ino allPlaces(after its emptied).
    Then filterPlace is also emptied for future searches. */
    ...
    /* When a place is clicked in list view, it triggers the marker click event.
     This sets the chosen place's marker to bounce, change color and
